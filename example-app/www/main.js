...
const colours = ['red', 'green', 'blue', 'yellow', 'pink', 'orange', 'purple', 'cyan'];
...
function log_for_watcher(text, time = Date.now(), colour = 'gray') {
  ...
  li.innerText = 'L' + timestamp(time) + ':W' + timestamp(Date.now()) + ':' + text;
...
function log_error(error, colour = 'gray') {
  console.error(error);
  return log_for_watcher(error.name + ': ' + error.message, Date.now(), colour);
}

function log_location(location, watcher_ID) {
  return log_for_watcher(location.latitude + ':' + location.longitude, location.time, watcher_colours[watcher_ID]);
}
  ...
  BackgroundGeolocation.addWatcher(
    ...
    remove_btn.onclick = function () {
      return BackgroundGeolocation.removeWatcher({ id })
        .then(function () {
          container.removeChild(container.children.item(Object.keys(watcher_colours).indexOf(id)));
...
function request_permissions() {
  capacitorLocalNotifications.LocalNotifications.requestPermissions().then(function (status) {
